...
def admin_panel_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📥 Ожидают", callback_data="panel_pending:0")],
...
def application_admin_keyboard(app_id: int, status: str = "pending"):
    rows=[]
    if status == "pending":
        rows.append([InlineKeyboardButton(text="✅ Одобрить", callback_data=f"app_approve:{app_id}"), InlineKeyboardButton(text="❌ Отклонить", callback_data=f"app_reject:{app_id}")])
