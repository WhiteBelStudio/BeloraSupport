...
def admin_panel_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="📥 Новые заявки", callback_data="panel_pending:0"),InlineKeyboardButton(text="📋 Все заявки", callback_data="panel_all:0")],[InlineKeyboardButton(text="📊 Статистика", callback_data="panel_stats"),InlineKeyboardButton(text="👥 Администраторы", callback_data="panel_admins")],[InlineKeyboardButton(text="🔄 Обновить", callback_data="panel_home")]])


def admin_management_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="➕ Добавить Telegram", callback_data="admin_add:telegram")],[InlineKeyboardButton(text="➕ Добавить VK", callback_data="admin_add:vk")],[InlineKeyboardButton(text="🗑 Удалить Telegram", callback_data="admin_del:telegram")],[InlineKeyboardButton(text="🗑 Удалить VK", callback_data="admin_del:vk")],[InlineKeyboardButton(text="🔄 Обновить список", callback_data="panel_admins")],[InlineKeyboardButton(text="⬅️ Назад", callback_data="panel_home")]])


def admin_list_keyboard(apps, page: int, total: int, prefix: str):
    rows=[[InlineKeyboardButton(text=f"#{row['id']} • {row['name']} • {row['status']}",callback_data=f"panel_app:{row['id']}")] for row in apps]
    nav=[]
    if page>0: nav.append(InlineKeyboardButton(text="⬅️",callback_data=f"{prefix}:{page-1}"))
    if (page+1)*10<total: nav.append(InlineKeyboardButton(text="➡️",callback_data=f"{prefix}:{page+1}"))
